...
def Teff(grav,temp,atm_bc):
    ...
    if atm_bc=="fortney2011a": 
        ...
        x = np.linspace(min(px),max(px),10)
        y = np.linspace(min(py),max(py),10)
        X,Y = np.meshgrid(x,y)
        
        teff_interp = griddata((px,py), f, (X,Y), method='cubic')
    ...
    if atm_bc=="fortney2011b":
        ...
        if np.isnan(teff_interp):
            K = 1.519
            expon_teff = 1.243
            return ((grav/100.0)**0.167*temp/K)**(1.0/expon_teff)  
        else:
            return teff_interp
